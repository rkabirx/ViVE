...
class Dual_thread(Thread):
    ...
    def __init__(self, conn, Assist_Port, Load_Port, CAN_Port):
        Thread.__init__(self)
        self.conn = conn
        self.Assist_Port = Assist_Port
...
#  socket connection for sending assist torque and load torque to motors
    def torque_send(self, x, y):
        ...
        TCP_PORT25 = self.Assist_Port		# 5025
        ...
        TCP_PORT11 = self.Load_Port				# 5044
    ...
    def rtc(self,x):
        ...
        TCP_PORT = self.Assist_Port			# 5025
...
def EPS_Sender_Main(Assist_Port, Load_Port, CAN_Port):
    ...
    while True:         # This is to make receiver from one socket and sender to other socket in 1 loop
        ...
        conn, addr = s.accept()
        conn.setblocking(0)
        thread1 = Dual_thread(conn, Assist_Port, Load_Port, CAN_Port)
        thread1.start()

if __name__ == '__main__':
    EPS_Sender_Main(Assist_Port, Load_Port, CAN_Port)
