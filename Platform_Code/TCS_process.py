...
class Dual_thread(Thread ):
    ...
    def __init__(self,conn, Output_Port ):
    ...
    def run(self ):
        ...
        if receiving_completed_flag == 1:
            ...
            TCP_PORT = self.Output_Port		#5003  # To connect to Hydraulic_modulator.py  # Port of HCU
            ...
            if (SlipRate > 0.3) & (Acc_position > 0) & (TCS_stat > 0) :
                ...
                print(f"TCP_Port to connect from TCP_Process to CAN bus is {TCP_PORT}")
...
def tcs_main(Output_Port) :
    ...
    print(f"In TCP_Process.py main function")
    s.listen(1)
    ...
    while True :  # This is to make receiver from one socket and sender to other socket in 1 loop
        ...
        conn,addr = s.accept()
        conn.setblocking(0)
        thread1 = Dual_thread(conn, Output_Port)
        thread1.start()


if __name__ == '__main__' :
    tcs_main(Output_Port)
