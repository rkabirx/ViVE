...
class Dual_thread(Thread):
    ...
    def __init__(self,conn, Hyd_Mod_PORT, CAN_PORT2):
        Thread.__init__(self)
        self.conn = conn
        self.Hyd_Mod_PORT = Hyd_Mod_PORT
        self.CAN_PORT2 = CAN_PORT2
    ...
    def run(self):
        ...
        if receiving_completed_flag == 1:
            ...
            # self.Hyd_Mod_PORT = 5010  # To connect to Hydraulic_modulator.py  # Port of HCU
            # self.CAN_PORT2 = 5003  # To connect to CAN Bus
            ...
            if (SlipRate > 0.3) & (brake_abs) > 0:
                ...
                s.connect((TCP_IP,self.Hyd_Mod_PORT))
                print(f"Connecting sender socket with port {self.Hyd_Mod_PORT}")
                ...
                s2.connect((TCP_IP,self.CAN_PORT2))
                print(f"Connecting sender socket with port {self.CAN_PORT2}")
...
class listener_thread(Thread):
    ...
    def run(self):
        ...
        print("-----------------------------------------------------------------")
        print('Starting a new connection in ABS listener thread')
...
def abs_send(CAN_Port):
    # This program generates and sends vehicle speed value as part of CAN frame from ABS to CAN simulator

    import socket
    import time

    TCP_IP = "0.0.0.0"
    TCP_PORT = CAN_Port	# 5003  # port for 1st payload (is_extended_id)
    BUFFER_SIZE = 1024
...
def ABS_ECU_Main(Hyd_Mod_PORT, CAN_PORT2):
    TCP_IP = "0.0.0.0"
    ...
    while True:  # This is to make receiver from one socket and sender to other socket in 1 loop
        ...
        conn,addr = s.accept()
        conn.setblocking(0)
        thread1 = Dual_thread(conn, Hyd_Mod_PORT, CAN_PORT2)
        thread1.start()


def ABS_ECU_Run_Threads(Hyd_Mod_PORT, CAN_PORT2):
    print("Starting ABS ECU sender")
    threading.Thread(target=ABS_ECU_Main, args=(Hyd_Mod_PORT, CAN_PORT2, )).start()
    print("Starting ABS ECU receiver")
